...
def calculate_flow(args, model, video):
    ...
    matched = similarity.similarityCheck(args.path)
        ...
        with torch.no_grad():
            for i in range(nFrame):
                if mode == 'forward':
                    ...
                    idx1,idx2,idx3 = matched[i]
                    # Flow i -> 0
                    print("Calculating {0} flow {1:2d} <---> {2:2d}".format(mode, i, idx1), '\r', end='')
                    image1 = video[i, None]
                    image2 = video[idx1, None]
                    FlowNLF0 = infer_flow(args, mode, '%05d_00000'%i, image1, image2, imgH, imgW, model, homography=False)

                    # Flow i -> nFrame // 2
                    print("Calculating {0} flow {1:2d} <---> {2:2d}".format(mode, i, idx2), '\r', end='')
                    image1 = video[i, None]
                    image2 = video[idx2, None]
                    FlowNLF1 = infer_flow(args, mode, '%05d_00001'%i, image1, image2, imgH, imgW, model, homography=False)

                    # # Flow i -> nFrame - 1
                    print("Calculating {0} flow {1:2d} <---> {2:2d}".format(mode, i, idx3), '\r', end='')
                    image1 = video[i, None]
                    image2 = video[idx3, None]
                    ...
                    idx1,idx2,idx3 = matched[i]
                    # Flow 0 -> i
                    print("Calculating {0} flow {1:2d} <---> {2:2d}".format(mode, idx1, i), '\r', end='')
                    image1 = video[idx1, None]
                    image2 = video[i, None]
                    FlowNLB0 = infer_flow(args, mode, '%05d_00000'%i, image1, image2, imgH, imgW, model, homography=False)

                    # Flow nFrame // 2 -> i
                    print("Calculating {0} flow {1:2d} <---> {2:2d}".format(mode, idx2, i), '\r', end='')
                    image1 = video[idx2, None]
                    image2 = video[i, None]
                    FlowNLB1 = infer_flow(args, mode, '%05d_00001'%i, image1, image2, imgH, imgW, model, homography=False)

                    # # Flow nFrame - 1 -> i
                    print("Calculating {0} flow {1:2d} <---> {2:2d}".format(mode, idx3, i), '\r', end='')
                    image1 = video[idx3, None]
                    image2 = video[i, None]
